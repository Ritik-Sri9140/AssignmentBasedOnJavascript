...
let salesMaster = [
  ...
  function updateSalesOrders(newSalesOrders) {
    newSalesOrders.forEach((order) => {
      const { id, quantity } = order;
      const product = salesMaster.find((item) => item.id === id);
      
      if (product) {
        product.sales.push(quantity);
      } else {
        salesMaster.push({ id, sales: [quantity] });
      }
    });
  }
  ...
  updateSalesOrders(newSalesOrders);
